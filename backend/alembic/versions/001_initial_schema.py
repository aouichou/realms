...
def upgrade() -> None:
    ...
    op.execute("DO $$ BEGIN CREATE TYPE charactertype AS ENUM ('player', 'companion', 'npc'); EXCEPTION WHEN duplicate_object THEN null; END $$;")
    op.execute("DO $$ BEGIN CREATE TYPE characterclass AS ENUM ('Fighter', 'Wizard', 'Rogue', 'Cleric'); EXCEPTION WHEN duplicate_object THEN null; END $$;")
    op.execute("DO $$ BEGIN CREATE TYPE characterrace AS ENUM ('Human', 'Elf', 'Dwarf', 'Halfling'); EXCEPTION WHEN duplicate_object THEN null; END $$;")
    ...
    op.create_table(
        ...
        sa.Column('character_type', postgresql.ENUM('player', 'companion', 'npc', name='charactertype', create_type=False), nullable=False, server_default='player'),
        sa.Column('character_class', postgresql.ENUM('Fighter', 'Wizard', 'Rogue', 'Cleric', name='characterclass', create_type=False), nullable=False),
        sa.Column('race', postgresql.ENUM('Human', 'Elf', 'Dwarf', 'Halfling', name='characterrace', create_type=False), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column('companion_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('characters.id', ondelete='SET NULL'), nullable=True),
        sa.Column('is_active', sa.Boolean(), nullable=False, server_default='true'),
        ...
        sa.Column('last_activity_at', sa.DateTime(), nullable=False, server_default=sa.func.now(), onupdate=sa.func.now())
    )
    ...
    op.create_index('ix_messages_session_created', 'conversation_messages', ['session_id', 'created_at'])
