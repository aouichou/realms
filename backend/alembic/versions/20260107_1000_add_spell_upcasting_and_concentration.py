...
def upgrade() -> None:
    ...
    op.add_column('spells', sa.Column('material_consumed', sa.Boolean(), nullable=False, server_default='false'))

    # Add active concentration tracking to characters table
    from sqlalchemy.dialects import postgresql
    op.add_column('characters', sa.Column('active_concentration_spell', postgresql.UUID(as_uuid=True), nullable=True))
