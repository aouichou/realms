...
@pytest.mark.asyncio
@pytest.mark.skip(reason="Stale mock — needs update for observability tracing layer")
...
async def test_chat_completion_rate_limit_error(mistral_client):
    ...
    mistral_client.client.chat.complete = Mock(side_effect=Exception("429 Rate limit exceeded"))
...
async def test_chat_completion_generic_error(mistral_client):
    ...
    mistral_client.client.chat.complete = Mock(side_effect=Exception("Network error"))
