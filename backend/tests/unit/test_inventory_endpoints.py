...
async def test_add_item_happy(client, db_session, auth_user):
    ...
    resp = await client.post(_inv_url(char.id, "/add"), json=body, headers=headers)
...
async def test_add_item_with_properties(client, db_session, auth_user):
    user, headers = auth_user
    char = make_character(user=user)
    db_session.add_all([char])
    await db_session.flush()
...
async def test_add_item_exceeds_weight_capacity(client, db_session, auth_user):
    user, headers = auth_user
...
async def test_add_item_exceeds_weight_with_existing_items(client, db_session, auth_user):
    ...
    db_session.add_all([char, existing])
    await db_session.flush()
...
async def test_add_item_equipped_true(client, db_session, auth_user):
...
async def test_add_item_memory_capture_with_session(client, db_session, auth_user):
...
async def test_add_item_memory_capture_failure_still_succeeds(client, db_session, auth_user):
...
async def test_get_inventory_empty(client, db_session, auth_user):
    user, headers = auth_user
    char = make_character(user=user)
    db_session.add_all([char])
    await db_session.flush()

    resp = await client.get(_inv_url(char.id), headers=headers)
...
async def test_get_inventory_with_items(client, db_session, auth_user):
    user, headers = auth_user
    char = make_character(user=user)
...
async def test_get_inventory_filter_by_type(client, db_session, auth_user):
    user, headers = auth_user
    char = make_character(user=user)
    ...
    resp = await client.get(_inv_url(char.id) + "?item_type=weapon", headers=headers)
    assert resp.status_code == 200
    data = resp.json()
...
async def test_get_inventory_filter_by_equipped(client, db_session, auth_user):
    user, headers = auth_user
    char = make_character(user=user)
    ...
    resp = await client.get(_inv_url(char.id) + "?equipped=true", headers=headers)
    assert resp.status_code == 200
    data = resp.json()
    assert len(data["items"]) == 1
...
async def test_get_inventory_weight_percentage(client, db_session, auth_user):
...
async def test_get_inventory_filter_by_type_and_equipped(client, db_session, auth_user):
    ...
    db_session.add_all([char, i1, i2, i3])
    await db_session.flush()

    resp = await client.get(_inv_url(char.id) + "?item_type=weapon&equipped=true", headers=headers)
    assert resp.status_code == 200
    data = resp.json()
    assert len(data["items"]) == 1
    assert data["items"][0]["name"] == "Equipped Sword"
...
async def test_toggle_equip_on(client, db_session, auth_user):
    user, headers = auth_user
    char = make_character(user=user)
    ...
    resp = await client.patch(_inv_url(char.id, f"/{item.id}/equip"), headers=headers)
    assert resp.status_code == 200
    assert resp.json()["equipped"] is True


async def test_toggle_equip_off(client, db_session, auth_user):
    user, headers = auth_user
    char = make_character(user=user)
...
async def test_toggle_equip_not_found(client, db_session, auth_user):
    user, headers = auth_user
    char = make_character(user=user)
    db_session.add_all([char])
    await db_session.flush()

    resp = await client.patch(_inv_url(char.id, f"/{uuid.uuid4()}/equip"), headers=headers)
    assert resp.status_code == 404


async def test_toggle_equip_wrong_character(client, db_session, auth_user):
    ...
    db_session.add_all([char1, char2, item])
    await db_session.flush()

    resp = await client.patch(_inv_url(char2.id, f"/{item.id}/equip"), headers=headers)
    assert resp.status_code == 404
...
async def test_update_item_quantity(client, db_session, auth_user):
    user, headers = auth_user
    char = make_character(user=user)
    ...
    resp = await client.patch(_inv_url(char.id, f"/{item.id}"), json={"quantity": 5}, headers=headers)
    assert resp.status_code == 200
    assert resp.json()["quantity"] == 5


async def test_update_item_properties(client, db_session, auth_user):
    user, headers = auth_user
    char = make_character(user=user)
    ...
    resp = await client.patch(_inv_url(char.id, f"/{item.id}"), json={"properties": new_props}, headers=headers)
    assert resp.status_code == 200
    assert resp.json()["properties"]["magic"] is True


async def test_update_item_quantity_zero_deletes(client, db_session, auth_user):
    user, headers = auth_user
    char = make_character(user=user)
    ...
    resp = await client.patch(_inv_url(char.id, f"/{item.id}"), json={"quantity": 0}, headers=headers)
    assert resp.status_code == 204


async def test_update_item_not_found(client, db_session, auth_user):
    user, headers = auth_user
    char = make_character(user=user)
    db_session.add_all([char])
    await db_session.flush()

    resp = await client.patch(_inv_url(char.id, f"/{uuid.uuid4()}"), json={"quantity": 1}, headers=headers)
    assert resp.status_code == 404


async def test_update_item_equipped_field(client, db_session, auth_user):
    ...
    resp = await client.patch(_inv_url(char.id, f"/{item.id}"), json={"equipped": True}, headers=headers)
    assert resp.status_code == 200
    assert resp.json()["equipped"] is True


async def test_update_item_multiple_fields(client, db_session, auth_user):
    ...
    resp = await client.patch(_inv_url(char.id, f"/{item.id}"), json=body, headers=headers)
    assert resp.status_code == 200
    data = resp.json()
...
async def test_update_item_negative_quantity_rejected(client, db_session, auth_user):
    ...
    resp = await client.patch(_inv_url(char.id, f"/{item.id}"), json={"quantity": -1}, headers=headers)
...
async def test_delete_item_happy(client, db_session, auth_user):
    user, headers = auth_user
    char = make_character(user=user)
    ...
    resp = await client.delete(_inv_url(char.id, f"/{item.id}"), headers=headers)
    assert resp.status_code == 204


async def test_delete_item_not_found(client, db_session, auth_user):
    user, headers = auth_user
    char = make_character(user=user)
    db_session.add_all([char])
    await db_session.flush()

    resp = await client.delete(_inv_url(char.id, f"/{uuid.uuid4()}"), headers=headers)
    assert resp.status_code == 404


async def test_add_multiple_items_weight_tracking(client, db_session, auth_user):
...
async def test_delete_item_wrong_character(client, db_session, auth_user):
    ...
    resp = await client.delete(_inv_url(char2.id, f"/{item.id}"), headers=headers)
    assert resp.status_code == 404
