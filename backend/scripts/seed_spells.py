    ...
    async def import_from_json(self, file_path: Path):
        ...
        for spell_data in spells:
            try:
                ...
                spell.school = self.normalizer.normalize_school(spell_data.get('school', 'Evocation'))
                spell.casting_time = self.normalizer.normalize_casting_time(spell_data.get('casting_time', '1 action'))
                ...
                # Description
                spell.description = self.normalizer.clean_description(spell_data.get('description', ''))
                spell.is_ritual = spell_data.get('ritual', False)
                
                # Classes
                classes = spell_data.get('classes', [])
                if classes:
                    spell.available_to_classes = {cls.lower(): True for cls in classes if cls in VALID_CLASSES}
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            ...
            for row in reader:
                try:
                    ...
                    if not spell.school:
                        spell.school = self.normalizer.normalize_school(row.get('school', 'Evocation'))
                    
                    if not spell.casting_time:
                        spell.casting_time = self.normalizer.normalize_casting_time(row.get('cast_time', '1 action'))
