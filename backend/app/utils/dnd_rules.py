...
def validate_asi_distribution(asi_distribution: dict, character_class: str, level: int) -> tuple[bool, str]:
    ...
    for level_str, increases in asi_distribution.items():
        ...
        if level_points != 2:
            return False, f"Each ASI must use exactly 2 points (level {asi_level} uses {level_points})"
        
        # Check that no single ability gets more than +2 at once
        for ability, points in increases.items():
            if points > 2 or points < 1:
                return False, f"Each ability increase must be +1 or +2 (got {points} for {ability} at level {asi_level})"
        
        total_points_used += level_points
    
    if total_points_used > total_points_allowed:
        return False, f"Used {total_points_used} ASI points but only {total_points_allowed} are available"
