...
async def get_asi_info(character_class: str, level: int):
    ...
    return ASIInfo(asi_count=asi_count, asi_levels=asi_levels, total_points=asi_count * 2)
...
async def get_all_racial_bonuses():
    ...
    """
    return [RacialBonusInfo(race=race, bonuses=bonuses) for race, bonuses in RACIAL_BONUSES.items()]
...
async def get_all_class_skills():
    ...
    return [
        ClassSkillInfo(
            character_class=class_name, skills_count=info["count"], available_skills=info["choices"]
        )
...
async def get_proficiency_bonus_by_level(level: int):
    ...
    return {"level": level, "proficiency_bonus": calculate_proficiency_bonus(level)}
