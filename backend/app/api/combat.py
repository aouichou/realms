...
from app.db.models import CombatEncounter, Character, Item, GameSession
...
async def start_combat(
    ...
    existing = await db.execute(
        select(CombatEncounter).where(
            and_(
                ...
                CombatEncounter.is_active == True
            )
        )
    )
    ...
    return CombatStatusResponse(
        ...
        current_participant=combat.participants[combat.current_turn] if combat.participants else None
    )
...
async def perform_combat_action(
    ...
    if action.action_type == CombatActionType.ATTACK:
        ...
        current_participant["hp_current"] = min(
            current_participant["hp_max"],
            current_participant["hp_current"] + healing
        )
...
@router.patch("/{combat_id}/participants/{participant_index}/hp", response_model=CombatStatusResponse)
async def update_participant_hp(
    ...
    participant["hp_current"] = max(0, min(participant["hp_max"], participant["hp_current"] + hp_change))
    new_hp = participant["hp_current"]
    
    combat.participants[participant_index] = participant
    
    if hp_change < 0:
        combat.combat_log.append(f"{participant['name']} takes {abs(hp_change)} damage! ({old_hp} → {new_hp} HP)")
    else:
        combat.combat_log.append(f"{participant['name']} is healed for {hp_change} HP! ({old_hp} → {new_hp} HP)")
