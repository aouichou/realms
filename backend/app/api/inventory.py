...
@router.post("/{character_id}/inventory/add", response_model=ItemResponse, status_code=status.HTTP_201_CREATED)
async def add_item(
    character_id: UUID,
    ...
    current_weight_result = await db.execute(
        select(func.coalesce(func.sum(Item.weight * Item.quantity), 0))
...
async def get_inventory(
    ...
    return InventoryResponse(
        ...
        weight_percentage=(current_weight / character.carrying_capacity * 100) if character.carrying_capacity > 0 else 0
    )
