...
from app.db.models import Quest, QuestObjective, CharacterQuest, Character, QuestState
...
class CreateQuestRequest(BaseModel):
    ...
    objectives: list[QuestObjectiveInput] = Field(..., min_items=1)
...
def create_quest(
    ...
    ).order_by(QuestObjective.order).all()
...
def get_character_quests(
    ...
    for quest in quests:
        ...
        responses.append(QuestResponse(
            id=quest.id,
            title=quest.title,
            description=quest.description,
            state=quest.state,
...
def complete_objective(
    ...
    objective = db.query(QuestObjective).filter(
    ...
    all_objectives = db.query(QuestObjective).filter(
        QuestObjective.quest_id == quest_id
    ).all()
...
def fail_quest(
    ...
    if quest.state in [QuestState.COMPLETED, QuestState.FAILED]:
        raise HTTPException(
            status_code=400,
            detail=f"Quest already {quest.state.value}"
        )
