...
CONDITION_EFFECTS = {
    ...
    "Frightened": {
        ...
        "effects": ["Disadvantage on checks/attacks while source visible", "Can't move closer to source"]
    },
    "Grappled": {
    ...
    "Invisible": {
        ...
        "effects": ["Attack rolls have advantage", "Attacks against have disadvantage", "Hide without cover"]
    },
    "Paralyzed": {
        "description": "A paralyzed creature is incapacitated and can't move or speak.",
        "effects": ["Incapacitated", "Auto-fail STR/DEX saves", "Attacks have advantage", "Hits within 5ft are crits"]
    },
    "Petrified": {
        ...
        "effects": ["Weight x10", "Stops aging", "Incapacitated", "Auto-fail STR/DEX saves", "Resistance to all damage", "Immune to poison/disease"]
    },
    "Poisoned": {
    ...
    "Prone": {
        ...
        "effects": ["Disadvantage on attacks", "Attacks within 5ft have advantage", "Attacks beyond 5ft have disadvantage"]
    },
    "Restrained": {
        "description": "A restrained creature's speed becomes 0, and it can't benefit from any bonus to its speed.",
        "effects": ["Speed = 0", "Disadvantage on attacks", "Disadvantage on DEX saves", "Attacks against have advantage"]
    },
    "Stunned": {
        "description": "A stunned creature is incapacitated, can't move, and can speak only falteringly.",
        "effects": ["Incapacitated", "Can't move", "Auto-fail STR/DEX saves", "Attacks against have advantage"]
    },
    "Unconscious": {
        "description": "An unconscious creature is incapacitated, can't move or speak, and is unaware of its surroundings.",
        "effects": ["Incapacitated", "Can't move/speak", "Drops held items", "Falls prone", "Auto-fail STR/DEX saves", "Attacks have advantage", "Hits within 5ft are crits"]
    },
}
...
async def add_condition(
    ...
    existing = db.query(CharacterCondition).filter(
...
async def remove_condition(
    ...
    condition = db.query(CharacterCondition).filter(
        CharacterCondition.id == condition_id,
        CharacterCondition.character_id == character_id
    ).first()
    ...
    return {
        "message": f"Condition {condition_name} removed",
        "character_id": character_id
    }
...
async def get_conditions(character_id: int, db: Session = Depends(get_db)):
    ...
    conditions = db.query(CharacterCondition).filter(
        CharacterCondition.character_id == character_id
    ).all()
