...
class DiceService:
    ...
    @staticmethod
    async def roll_dice(
        notation: str, roll_type: RollType = RollType.NORMAL
        ...
        if roll_type == RollType.ADVANTAGE:
            ...
            if count != 1:
                raise ValueError(
                    "Advantage/disadvantage only works with single die rolls (e.g., d20)"
                )
    ...
    def format_breakdown(
        notation: str, rolls: list[DiceRollResult], modifier: int, total: int
    ) -> str:
