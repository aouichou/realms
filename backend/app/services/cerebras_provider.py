"""
Cerebras Provider
Implements AIProvider interface for Cerebras API (OpenAI-compatible)
Super fast inference with unlimited soft limits
"""
...
class CerebrasProvider(AIProvider):
    """Cerebras provider implementation (OpenAI-compatible)"""
    ...
    def __init__(
        ...
        super().__init__(name="cerebras", priority=priority)
        ...
        logger.info(f"Initialized Cerebras provider with model: {model}")
    ...
    async def generate_narration(
        ...
        **kwargs,
        ...
        """
        Generate narration using Cerebras
    ...
    async def generate_chat(
        ...
        **kwargs,
        ...
        try:
            ...
            if not content:
                raise ProviderUnavailableError("Cerebras returned empty response")
            ...
            logger.warning(f"Cerebras rate limit exceeded: {e}")
            ...
            raise ProviderUnavailableError(f"Cerebras provider error: {str(e)}")

        except Exception as e:
            duration = time.time() - start_time
            metrics.record_llm_request(model=self.model, status="error", duration=duration)
            logger.error(f"Unexpected Cerebras error: {e}")
            self.set_status(ProviderStatus.ERROR, str(e))
            raise ProviderUnavailableError(f"Cerebras provider error: {str(e)}")

    async def is_available(self) -> bool:
        """
        Check if Cerebras is currently available
