...
from app.db.models import Character, CharacterClass, CharacterType, Item
...
class CharacterService:
    ...
    async def create_character(
        db: AsyncSession, character_data: CharacterCreate, user_id: Optional[UUID] = None
    ) -> Character:
        ...
        if character_data.skill_proficiencies:
            is_valid, error_msg = validate_skill_selection(
                character_data.skill_proficiencies, character_data.character_class
            )
            if not is_valid:
        ...
        hp_max = CharacterService.calculate_hp_max(
            character_data.character_class, final_scores["constitution"], character_data.level
        )
    ...
    @staticmethod
    async def calculate_character_stats(db: AsyncSession, character_id: UUID) -> Optional[dict]:
        ...
        # Get equipped items
        result = await db.execute(
            select(Item).where(Item.character_id == character_id, Item.equipped.is_(True))
        )
