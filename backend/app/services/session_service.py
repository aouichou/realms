...
class GameSessionService:
    ...
    async def create_session(
        db: AsyncSession, user_id: Optional[UUID], session_data: SessionCreate
    ) -> GameSession:
    ...
    async def get_user_sessions(
        db: AsyncSession, user_id: UUID, active_only: bool = False, skip: int = 0, limit: int = 100
    ...
    @staticmethod
    async def get_active_session(db: AsyncSession, user_id: UUID) -> Optional[GameSession]:
    ...
    async def end_session(db: AsyncSession, session_id: UUID) -> Optional[GameSession]:
        ...
        return await GameSessionService.update_session(
            db, session_id, SessionUpdate(is_active=False)
        )
