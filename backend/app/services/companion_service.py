...
class CompanionService:
    ...
    def _format_prompt(
        ...
        prompt_template = COMPANION_PERSONALITIES.get(personality, {}).get(
            "system_prompt", 
            COMPANION_PERSONALITIES["helpful"]["system_prompt"]
        )
    ...
    async def generate_companion_speech(
        ...
        if not client:
            ...
            fallback = {
                "helpful": f"Let me help you with that. Stay safe out there!",
                "brave": f"We can handle this! To victory!",
                "cautious": f"Perhaps we should proceed carefully...",
                "sarcastic": f"Oh great, another {trigger.replace('_', ' ')}...",
                "mysterious": f"I sense something... unusual here.",
                "scholarly": f"Interesting. According to my studies, this is quite rare."
            }
