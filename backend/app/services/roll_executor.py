...
class DiceRollResult:
    ...
    def is_critical(self) -> bool:
        ...
        return self.notation.startswith("d20") and len(self.rolls) > 0 and max(self.rolls) == 20
...
class RollExecutor:
    ...
    DICE_PATTERN = re.compile(r"(\d*)d(\d+)(?:dl(\d+)|dh(\d+))?", re.IGNORECASE)
    ...
    def _parse_notation(cls, notation: str) -> tuple[list[dict], int]:
        ...
        if not dice_parts:
            dice_parts.append({"count": 1, "sides": 20, "drop_lowest": 0, "drop_highest": 0})

        return dice_parts, modifier

    @classmethod
    def _roll_dice(cls, dice_parts: list[dict], advantage: bool, disadvantage: bool) -> list[int]:
        """
