...
export function CombatTracker({
  ...
  const fetchCombatStatus = async (combatId: string) => {
    try {
      const response = await fetch(
        `http://localhost:8000/api/combat/${combatId}/status`
      );
      const data = await response.json();
      setCombatStatus(data);
    } catch (error) {
  ...
  const performAction = async (
    ...
    try {
      const response = await fetch(
        `http://localhost:8000/api/combat/${combatStatus.combat_id}/action`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  const updateParticipantHP = async (
    ...
    try {
      await fetch(
        `http://localhost:8000/api/combat/${combatStatus.combat_id}/participants/${participantIndex}/hp`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
  ...
  const endCombat = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8000/api/combat/${combatStatus.combat_id}/end`,
        {
          method: "POST",
        }
      );
