...
export function InventoryPanel({ characterId }: InventoryPanelProps) {
  ...
  const fetchInventory = async () => {
    try {
      ...
      const response = await fetch(
        `http://localhost:8000/api/characters/${characterId}/inventory?${params}`
      );
  ...
  const toggleEquip = async (itemId: string) => {
    try {
      await fetch(
        `http://localhost:8000/api/characters/${characterId}/inventory/${itemId}/equip`,
        { method: "PATCH" }
      );
      fetchInventory();
    } catch (error) {
  ...
  const deleteItem = async (itemId: string) => {
    try {
      await fetch(
        `http://localhost:8000/api/characters/${characterId}/inventory/${itemId}`,
        { method: "DELETE" }
      );
